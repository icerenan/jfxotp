...
public class GoogleAuthenticatorUtils {
    ...
    public static String genSecret(String user) {
        ...
        GoogleAuthenticatorUtils.getQRBarcodeURL(user,
                "testhost", secret);
        return secret;
    }
    ...
    private static long getCodeLong(String secret){
        Base32 codec = new Base32();
        byte[] key = codec.decode(secret);
        long t = (System.currentTimeMillis() / 1000L) / 30L;
        try {
            long hash = verify_code(key,t);
            return hash;
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
    ...
    public static String getCode(String secret){
        return Long.toString(getCodeLong(secret));
    }
